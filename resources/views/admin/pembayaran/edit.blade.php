                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span class="font-medium text-green-600">Rp
                            {{ number_format($pembayaran->tagihan->total_tagihan, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('pembayaran.update', $pembayaran->id_pembayaran) }}"
                id="editPembayaranForm">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Tanggal Pembayaran -->
                    <div>
                        <label for="tanggal_pembayaran" class="block text-sm font-medium text-gray-700 mb-2">Tanggal
                            Pembayaran</label>
                    ...
                    <!-- Biaya Admin -->
                    <div>
                        <label for="biaya_admin" class="block text-sm font-medium text-gray-700 mb-2">Biaya Admin
                            (Rp)</label>
                        <input type="number" id="biaya_admin" name="biaya_admin"
                            value="{{ old('biaya_admin', $pembayaran->biaya_admin) }}" min="0" step="100"
                            required
                        ...
                        <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                                ...
                                <div>
                                    ...
                                    <span
                                        class="font-medium">{{ date('d/m/Y H:i', strtotime($pembayaran->created_at)) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
