    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <form method="POST" action="{{ route('tagihan.store') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Penggunaan (Optional) -->
                <div class="md:col-span-2">
                    <label for="id_penggunaan" class="block text-sm font-medium text-gray-700 mb-2">Penggunaan (Opsional)</label>
                    <select id="id_penggunaan" name="id_penggunaan"
                        ...
                        <option value="{{ $penggunaan->id_penggunaan }}" {{ old('id_penggunaan') == $penggunaan->id_penggunaan ? 'selected' : '' }}>
                            {{ $penggunaan->pelanggan->nama_pelanggan }} - {{ DateTime::createFromFormat('!m', $penggunaan->bulan)->format('F') }} {{ $penggunaan->tahun }} ({{ $penggunaan->jumlah_meter }} kWh)
                        </option>
                        @endforeach
                    </select>
                    @error('id_penggunaan')
                ...
                <div>
                    ...
                    <input type="number" id="tahun" name="tahun" value="{{ old('tahun', date('Y')) }}" min="2020" max="2030" required
                ...
                <!-- Jumlah Meter -->
                <div>
                    <label for="jumlah_meter" class="block text-sm font-medium text-gray-700 mb-2">Jumlah Meter (kWh)</label>
                    <input type="number" id="jumlah_meter" name="jumlah_meter" value="{{ old('jumlah_meter') }}" min="0" required
